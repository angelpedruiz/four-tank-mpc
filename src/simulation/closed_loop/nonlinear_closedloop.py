...
from utils.piecewise import create_piecewise_cnst_seq



def main():
    """Main function for running nonlinear closed-loop simulation with PID control."""
    ...
    fig.suptitle('Four Tank System - Nonlinear Closed-Loop Simulation with PID', fontsize=16)
    ...
    print("\nNonlinear Closed-Loop Simulation with PID Control completed!")
    print("Model components implemented:")
    print("1. Deterministic dynamics: x_dot(t) = f(x(t),u(t),d(t),p)")
    print("2. Measurement model: y(t) = g(x(t),p) + v(t) with v(t) ~ N(0,Rvv(p))")
    print("3. Output model: z(t) = h(x(t),p)")
    print("4. PID control: u(t) = Kp*e(t) + Ki*integral(e(t)) + Kd*d/dt(e(t))")
    print("\nFinal results:")
